...
def getRep(imgPath, multiple=False):
    imgPath1 = os.path.abspath(imgPath)
    start = time.time()
    print imgPath1
    bgrImg = cv2.imread(imgPath1)
    print bgrImg
    if bgrImg is None:
        return None
...
def train(args):
    ...
    fname = "{}/labels.csv".format("/home/ankush/openface/generated-embeddings")
    labels = pd.read_csv(fname, header=None).as_matrix()[:, 1]
    labels = map(itemgetter(1),
                 map(os.path.split,
                     map(os.path.dirname, labels)))  # Get the directory.
    fname = "{}/reps.csv".format("/home/ankush/openface/generated-embeddings")
    ...
    # trainParser.classifier == 'LinearSvm'
    ...
    # if args.ldaDim > 0:
    clf_final = clf
    clf = Pipeline([('lda', LDA(n_components=1)),
                    ('clf', clf_final)])

    clf.fit(embeddings, labelsNum)

    fName = "{}/classifier.pkl".format("/home/ankush/openface/generated-embeddings")
...
def infer(args, multiple=False):
    print args
    imagepath=os.path.abspath(args)
    print imagepath
    with open("/home/ankush/openface/generated-embeddings/classifier.pkl", 'rb') as f:
    ...
    for img in args:
        ...
        for r in reps:
            ...
            cursor = db.cursor()
            sql1 = "SELECT user_name FROM data WHERE user_id = ('%s')" % (person)
            cursor.execute(sql1)
            ...
            if multiple:
                ...
                if confidence <= 0.80:
                    ...
                    print confidence
                    wrongface="wrong face found"
                    return ({'please try again' : wrongface})
                else :
                    confidence=confidence*100
                    pre = "Welcome {}".format(ids)
                    print pre
                    print("Predict {} with {:.2f} confidence.".format(ids, confidence))
                    return ({'name' : pre , 'Confidence' : confidence})
...
if __name__ == '__main__':
    ...
    trainParser.add_argument(
        ...
        default='LinearSvm')
    trainParser.add_argument(
        '--workDir',default='/home/ankush/openface/generated-embeddings',
        type=str,
